...
from myutils import pos_caption_to_words, predeined_set_caption_to_words, sample_batch
# from llava.eval.aggreagate import extract_objects
import argparse
import torch
import os
...
def cal_selfconsistency(args, model, tokenizer, images_tensor, caption, objects_set=None): 
    
    objects = evaluator.caption_to_words(caption)[0] 
    if objects_set is not None:
        
        print("=============================")
        print("objects_set:", objects_set)
        objects = predeined_set_caption_to_words(caption, objects_set)
    print("caption:", caption)
    print("to measure objects:", objects)
    ...
    return score
def sample_best(args, model ,tokenizer, image_tensor, captions, most_common_objects=None):
    
    score_list = []

    for caption in captions: 
        objects_set = None
        if args.extract_method == "model": 
            objects_set = set(extract_objects(args, model, tokenizer, "||".join(captions)))
        elif args.extract_method == "statis":
            objects_set = most_common_objects
        score = cal_selfconsistency(args, model, tokenizer, image_tensor, caption, objects_set) 
...
def statis_objects(args, model, tokenizer):
    ...
    if os.path.exists("most_common_objects.json"):
        with open("most_common_objects.json", 'r') as f:
            most_common_objects = json.load(f)
        return most_common_objects
    with open('detail_23k.json', 'r') as f:  
        detail_data = random.sample(json.load(f),400)
        captions = (item['conversations'][1]['value'] for item in detail_data)
    all_objects = []
    ...
    most_common_objects = count.most_common(150)
    most_common_objects = [obj[0] for obj in most_common_objects if obj[0] not in ['image', 'scene']]
    print("most_common_objects:", most_common_objects)

    
    with open("most_common_objects.json", 'w') as f:
...
def sample_caption_model(args): 
    ...
    global sampling_time, verification_time
    ...
    for line in tqdm(questions):
        ...
        start_time = time.time()
        best_caption = sample_best(args, model, tokenizer, image_tensor, outputs_list, most_common_objects=most_common_objects) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--answers-file", type=str, default="./experiments/out/run_times10/bon_rd1.jsonl")
    parser.add_argument("--generate-mode", type=str, choices=["glm4v", "model"],default="model")
    ...
    print("sampling_time:", sampling_time, "verification_time:", verification_time)